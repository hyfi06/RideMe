...
function authApi(app) {
    ...
    passport.authenticate('basic', (error, user) => {
      try {
        ...
        req.login(user, { session: false }, async (error) => {
          ...
          const { _id: id, first_name, last_name, email } = user;
          ...
          return res.status(200).json({ token, 'user': { id, first_name, first_name, email } });
        });
      } catch (error) {
        next(error);
      }
  ...
  router.post('/sign-provider', validationHandler(createProviderUserSchema), async (req, res, next) => {
    ...
    try {
      ...
      const { _id: id, first_name, last_name, email } = queriedUser;
      ...
      return res.status(200).json({ token, user: { id, first_name, last_name, email } });
    } catch (error) {
      next(error);
    }
  });
